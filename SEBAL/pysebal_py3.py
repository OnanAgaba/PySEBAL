...
class CSVWorkbook:
    """Mock workbook class for CSV input"""
    def __init__(self, csv_data, row_index):
        self.csv_data = csv_data
        self.row_index = row_index
        self.sheets = {
            'General_Input': CSVWorksheet(csv_data, row_index, 'General_Input'),
            'Meteo_Input': CSVWorksheet(csv_data, row_index, 'Meteo_Input'),
            'Landsat_Input': CSVWorksheet(csv_data, row_index, 'Landsat_Input'),
            'Soil_Input': CSVWorksheet(csv_data, row_index, 'Soil_Input')
        }
    
    def __getitem__(self, sheet_name):
        return self.sheets.get(sheet_name, CSVWorksheet(self.csv_data, self.row_index, sheet_name))

class CSVWorksheet:
    """Mock worksheet class for CSV input"""
    def __init__(self, csv_data, row_index, sheet_name):
        self.csv_data = csv_data
        self.row_index = row_index
        self.sheet_name = sheet_name
        # Column mapping for different sheets
        self.column_mapping = {
            'General_Input': {
                'B': 'input_folder',
                'C': 'output_folder', 
            ...
            'Landsat_Input': {
                ...
                'I': 'ndvicold_low',
                'J': 'ndvicold_high',
                'K': 'Hot_Pixel_Constant',
                'L': 'Cold_Pixel_Constant'
            },
            'Meteo_Input': {
                'B': 'Temp_inst',
                'C': 'Temp_24',
    ...
    def __getitem__(self, cell_ref):
        return CSVCell(self.csv_data, self.row_index, self.sheet_name, cell_ref, self.column_mapping)

class CSVCell:
    """Mock cell class for CSV input"""
    def __init__(self, csv_data, row_index, sheet_name, cell_ref, column_mapping):
        self.csv_data = csv_data
        self.row_index = row_index
        self.sheet_name = sheet_name
    ...
    @property
    def value(self):
        # Extract column letter from cell reference (e.g., 'B2' -> 'B')
        col_letter = ''.join([c for c in self.cell_ref if c.isalpha()])
        
        # Get column name from mapping
        sheet_mapping = self.column_mapping.get(self.sheet_name, {})
        column_name = sheet_mapping.get(col_letter)
        
        if column_name and column_name in self.csv_data.columns:
            try:
                value = self.csv_data.iloc[self.row_index][column_name]
                return None if pd.isna(value) else value
            except (IndexError, KeyError):
                return None
        return None

def load_csv_as_workbook(csv_file, row_index):
    """Load CSV file and return a workbook-like object"""
    try:
        csv_data = pd.read_csv(csv_file)
        return CSVWorkbook(csv_data, row_index)
